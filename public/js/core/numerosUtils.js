define(['underscore'], function( _ ) {
	...
	var Numero = function (options) {};

	Numero.prototype.convert = function(numero) {

		this.mod = function (dividendo, divisor) {
		...
		this.ObtenerParteEntDiv = function (dividendo, divisor) {
			var resDiv = dividendo / divisor ;
		...
		this.fraction_part = function (dividendo, divisor) {
			var resDiv = dividendo / divisor;
		...
		this.string_literal_conversion = function (number) {
			...
			var centenas = this.ObtenerParteEntDiv(number, 100);
			number = this.mod(number, 100);
			var decenas = this.ObtenerParteEntDiv(number, 10);
			number = this.mod(number, 10);

			var unidades = this.ObtenerParteEntDiv(number, 1);
		...
		this.convertirNumLetras = function (number) {
			//number = number_format (number, 2);
			var number1=number.toString();
			//settype (number, "integer");
			var cent = number1.split(".");
			var centavos = (cent.length==1)? 0 : cent[1];
			var centenas_final_string, millions_final_string, thousands_final_string, centenas, separador, cad;
			var separar = false;
			if(cent.length > 1){
				separar = (cent[1] > 0)? true : false;
			}
			var incluir_cero = false;
			...
			{
				var millions  = this.ObtenerParteEntDiv(number, 1000000); // first, send the millions to the string
				number = this.mod(number, 1000000);           // conversion function
			...
			//console.log("DECIMAL: ",centavos);
			centavos = (centavos === 0 || centavos === "00" || centavos === "0")? "CERO" : this.string_literal_conversion(centavos);

			if(centavos.trim() == "un" || centavos.trim() == "UN")
				centavos = "UNO";

			centavos = centavos.toUpperCase();

			if(centavos === "CERO" && incluir_cero === false)
				centavos = "";

			/* Asigna el tipo de separador, para 1 = PESO, para distinto de 1 = PESOS*/
			if (separar === true)
			{
				separador = " PUNTO ";
			}
			...
			/* Regresa el número en cadena entre paréntesis y con tipo de separador y la fase M.N.*/
			...
			var letra_num = cad+separador+centavos;

			return letra_num.trim();
		};

		return (numero > 0)? this.convertirNumLetras(numero) : "CERO";
	};
